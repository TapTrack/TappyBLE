...
public class HDLCByteArrayParser {

    public static HDLCParseResult process(byte[] bytes) {
        if(bytes == null)
            throw new NullPointerException("Cant load a null byte array");
        if(bytes.length > 0) {
            List<byte[]> packets = new ArrayList<byte[]>(bytes.length / 20);
